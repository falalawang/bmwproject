	...
	function checkCombo(obj){
		$(".selectCombo").removeClass("hidden");//当单击某个车型时,选套餐提示显示
   ...
   //  	//alert($("#appointment").html(app));
  	...
  	$("#submit").click(function(){	//提交订单之前做的判断
	  	if($("#tel").val() && $("#carType").val() && $("#orderdate").val() && $("#ordertime").val() && $("#user").val() && $("#code").val() && $("#carNumber").val()){		//判断加*是否为空,为空不能提交
            ...
            if($("#lastPrice").html() <= 0){
                location.hash="carAnchor";
                alert("请选择套餐")
                return false;
            }
            if(!$("#carNumber").val()){
                $("#carNumber").css("border-color","red");
                location.hash="carNumber";
                //获取当前屏幕的高度
  ...
  function setCookie(name, value, iDay){
    ...
    $("#tel").bind('input propertychange',function(){				//输入手机号时鼠标抬起触发的事件
		var tel=document.getElementById("tel");
		//当输入了11位合法手机号时,触发发送验证码按钮,变换背景色提示用户发送验证码,取消禁用
		if(tel.value.length==11){
			$("#btn").addClass('btn-primary').attr('disabled',false);
		}else{
